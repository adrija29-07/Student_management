  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      {activity.filePath && (() => {
        ...
        const decodedName = activity.filePath.startsWith('http')
          ? decodeURIComponent(activity.filePath.split('/').pop()?.replace(/^\d+-/, '') || 'Uploaded File')
          : activity.filePath.replace(/^\d+-/, '');
        const fileName = decodedName || 'Uploaded File';
        const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(fileUrl) || fileUrl.includes('imagekit.io');

        return (
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
            <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Uploaded Certificate / File</h3>
            {isImage ? (
              <div className="mb-4">
                <a href={fileUrl} target="_blank" rel="noreferrer">
                  <img
                    ...
                    className="w-full max-h-96 object-contain rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 cursor-pointer hover:opacity-90 transition-opacity"
                    onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                  />
                </a>
                <p className="text-xs text-slate-400 mt-2 text-center">Click image to open full size</p>
              </div>
            ) : (
              <div className="mb-4 text-sm text-slate-500 dark:text-slate-400">
                Preview is not available for this file type. Use the open/download buttons below.
              </div>
            )}
            <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-slate-700 dark:text-slate-300 truncate">{fileName}</p>
                <p className="text-xs text-slate-400">{isImage ? 'Image file' : 'Uploaded document'}</p>
              </div>
