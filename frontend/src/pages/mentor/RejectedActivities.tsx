...
import { XCircle, User, Award, AlertCircle, Loader2, Calendar, Eye } from 'lucide-react';
...
export const RejectedActivities: React.FC = () => {
  const navigate = useNavigate();
  const [data, setData] = useState<Approval[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    mentorAPI.getRejectedActivities()
    ...
    <div className="space-y-5 max-w-5xl mx-auto">
      <div>
        <h2 className="text-2xl font-outfit font-bold text-slate-800 dark:text-white flex items-center gap-2">
          <XCircle className="h-6 w-6 text-rose-500" /> Rejected Activities
        </h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">{data.length} activities rejected by you</p>
      </div>
      {data.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl">
          <AlertCircle className="mx-auto h-10 w-10 text-slate-300 dark:text-slate-700 mb-3" />
          <p className="text-slate-500 dark:text-slate-400 text-sm">No rejected activities</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {data.map(a => (
            <div key={a.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:border-rose-300 dark:hover:border-rose-700 transition-colors">
              <div className="flex items-start gap-4">
                <div className="h-10 w-10 rounded-xl bg-rose-50 dark:bg-rose-950/20 flex items-center justify-center flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 text-xs font-bold text-slate-500 dark:text-slate-400 flex-shrink-0">
                  ...
                  {a.feedback && (
                    <div className="mt-1.5 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-800 rounded-lg px-3 py-2">
                      <p className="text-xs text-rose-700 dark:text-rose-300 italic">Reason: "{a.feedback}"</p>
                    </div>
                  )}
                </div>
                <button
